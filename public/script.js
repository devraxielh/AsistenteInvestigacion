/********************************************
 * Asistente de Investigación - Frontend
 * Se comunica con el backend (server.js)
 ********************************************/
...
/**
 * Llama al backend en /api/gemini
 */
async function consultarGemini(texto, idDiv) {
    try {
        ...
        if (data.error) {
            document.getElementById(idDiv).innerText = "Error: " + data.error;
            return;
        }
        ...
        document.getElementById(idDiv).innerText = "Error al conectar con el servidor.";
    }
}
